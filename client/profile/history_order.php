            ...
            <div class="box">
                ...
                <div class="box_2">
                    <table>
                        ...
                        <tbody>
                            <?php
                            $cart_query = mysqli_query($conn, "SELECT * FROM `orders` WHERE user_id = '$user_id'") or die('query failed');
                            if (mysqli_num_rows($cart_query) > 0) {
                                while ($fetch_product = mysqli_fetch_assoc($cart_query)) {
                            ?>
                                    <tr>
                                        <td><?php echo $fetch_product['id']; ?></td>
                                        <td><?php echo $fetch_product['customer_name']; ?></td>
                                        <td><?php echo $fetch_product['quantity_product']; ?></td>
                                        <td><?php
                                            if ($fetch_product['order_status'] === "treatment") {
                                                echo "Обработка";
                                            } elseif ($fetch_product['order_status'] === "sent") {
                                                echo "Відправлено";
                                            } else {
                                                echo "Сделка завершена";
                                            }
                                            ?></td>
